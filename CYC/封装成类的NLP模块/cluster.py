...
def main_tops(ans_file_path, top_number):
    ...
    for word in words:
        ...
        predict = predict[0]
        predict_cluster = clusters[predict]
        VoteWeight[word] = SumClusterDegree[predict] * np.linalg.norm(worddict[word] - predict_cluster) / \
                           SumClusterDistance[predict]
    ...
    top_list = []
    for i in sortk:
        if sdrow[i[1]] not in stopwords and len(sdrow[i[1]]) >= 2:
